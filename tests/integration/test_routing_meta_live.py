...
def live_setup() -> Generator[tuple[Client, str, str, str, str], None, None]:
    """Initialize the client with real environment variables and ensure safe teardown."""
    ...
    yield client, domain, messages_from, messages_to, validation_address_1

    # Teardown: purge the session pool to prevent socket leaks
    client.close()
...
class TestLiveRoutingMeta:
    def test_intelligent_routing_to_mailgun_servers(
        ...
        # Routes that legitimately 404 without valid params or due to Sandbox limits
        ...
        for ep_name, caller in sorted(test_calls.items()):
            try:
                ...
                if status == 404 and ep_name in expected_404_routes:
                    ...
                    routing_crashes.append((ep_name, str(url)))
                else:

...
class AsyncDomainTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_single_domain(self) -> None:
        # Do not try to create a domain here. Use the globally configured self.domain
        req = await self.client.domains.get(domain_name=self.domain)
...
class AsyncMailingListsTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_maillists_lists_delete(self) -> None:
        target_address = f"python_sdk_async@{self.domain}"

        # Ensure async data dictionary points to this unique async runtime namespace
        self.mailing_lists_data["address"] = target_address
